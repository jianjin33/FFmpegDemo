...
typedef struct MyAVPacketList {
    ...
    int serial; // 可以理解成操作数。在初始化和快进的时候，会增加操作数。小于的操作数，在下一次显示的时候，会直接被抛弃。
} MyAVPacketList;
...
typedef struct VideoState {
    ...
    AVFilterGraph *agraph;              // audio filter graph // 音频过滤器 
#endif
    // 上一个视频码流Id、上一个音频码流Id、上一个字幕码流Id
    int last_video_stream, last_audio_stream, last_subtitle_stream;

    SDL_cond *continue_read_thread; // read_thread的条件变量
} VideoState;
...
static void packet_queue_start(PacketQueue *q)
{
    SDL_LockMutex(q->mutex);
    q->abort_request = 0;
    packet_queue_put_private(q, &flush_pkt); // 将flush_pkt送入PacketQueue队列当中。 生产者av_read_frame 不断的放进这个Queue中
    SDL_UnlockMutex(q->mutex);
}
    ...
    for (;;) {
            ...
            do {
                ...
                switch (d->avctx->codec_type) {
                    case AVMEDIA_TYPE_VIDEO:
                        ret = avcodec_receive_frame(d->avctx, frame);   // ffmpeg很熟悉
                        if (ret >= 0) {
                            if (decoder_reorder_pts == -1) {
                                // frame的pts可以通过best_effort_timestamp来取的 下面分支的音频通过pkt_timebase 时间基来计算
        ...
        do {
            ...
            if (d->packet_pending) {
                ...
                if (packet_queue_get(d->queue, &pkt, 1, &d->pkt_serial) < 0) // Queue中去获取pkt 消费
                    return -1;
            }
...
/* display the current picture, if any */ // 同步后时间，取到具体的frame时，就送入显示。
static void video_display(VideoState *is)
{
    //video_open 先去创建SDL_Window和SDL_Renderer。
    ...
    // 参考：作者：deep_sadness 链接：https://www.jianshu.com/p/088dd1c32942
    if (!is->width)
        video_open(is);
    ...
    // 显示步骤4 
        ...
        video_image_display(is); // video部分
    // 显示步骤5
    SDL_RenderPresent(renderer);    // SDL去显示
}
...
static int video_thread(void *arg)
{
    ...
    // 开始循环解码
    for (;;) {
        ret = get_video_frame(is, frame); // v5 具体的获取frame，ffmpeg的api
        if (ret < 0)
            goto the_end;
        if (!ret)
            continue;
        ...
        while (ret >= 0) {
            ...
            ret = queue_picture(is, frame, pts, duration, frame->pkt_pos, is->viddec.pkt_serial); // 得到AvFrame 放入到已解码队列 FrameQueue
            av_frame_unref(frame);
#if CONFIG_AVFILTER
...
{   
    ...
    avctx = avcodec_alloc_context3(NULL); // ffmpeg很熟悉 4
    if (!avctx)
        return AVERROR(ENOMEM);

    ret = avcodec_parameters_to_context(avctx, ic->streams[stream_index]->codecpar);    // ffmpeg很熟悉 5
    if (ret < 0)
        goto fail;
    avctx->pkt_timebase = ic->streams[stream_index]->time_base;

    codec = avcodec_find_decoder(avctx->codec_id); // ffmpeg很熟悉 6
    ...
    case AVMEDIA_TYPE_VIDEO:    
        ...
        // v4.  正式开启解码线程 从read_thread线程 -> video_thread解码线程
...
static int read_thread(void *arg)
{
    ...
    ic = avformat_alloc_context();  // ffmpeg很熟悉 1
    if (!ic) {
    ...
    err = avformat_open_input(&ic, is->filename, is->iformat, &format_opts); // ffmpeg很熟悉 2
    if (err < 0) {
    ...
    if (find_stream_info) {
        ...
        err = avformat_find_stream_info(ic, opts); // ffmpeg很熟悉 3
    ...
    // 8. 找到视频流之后，还会再次同步显示的比例和视频的长宽
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        AVStream *st = ic->streams[st_index[AVMEDIA_TYPE_VIDEO]];
        AVCodecParameters *codecpar = st->codecpar;
        AVRational sar = av_guess_sample_aspect_ratio(ic, st, NULL);
        if (codecpar->width)
            set_default_window_size(codecpar->width, codecpar->height, sar);    // 同步视频宽高
    }

    /* open the streams */
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0) {
        stream_component_open(is, st_index[AVMEDIA_TYPE_AUDIO]); // 9-a1. 打开解码线程（音频） 
    }

    ret = -1;
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ret = stream_component_open(is, st_index[AVMEDIA_TYPE_VIDEO]);  // 9-v1. 打开解码线程（视频）
    }
...
//      of the seek_pos/seek_rel variables
            ...
            is->seek_req = 0;
            is->queue_attachments_req = 1;  // 比如说一些MP3，AAC音频文件附带的专辑封面。所以，就是如果有，就去显示吧。
            is->eof = 0;
            if (is->paused)
        ...
        ret = av_read_frame(ic, pkt);   // 12. 读取pkt放入各个类型的队列中
        if (ret < 0) {
            ...
            if (ic->pb && ic->pb->error)
                break;
            SDL_LockMutex(wait_mutex);
            SDL_CondWaitTimeout(is->continue_read_thread, wait_mutex, 10); // 读取失败等待10ms
            SDL_UnlockMutex(wait_mutex);
            continue;
        } else {
            is->eof = 0;
        ...
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            ...
            packet_queue_put(&is->videoq, pkt); // 13-v putQueue
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    ...
    while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) {
        ...
        remaining_time = REFRESH_RATE;  // 视频需要根据pts来显示，之间有间隔，正常进行的话，还需要间隔固定的时间。这里就是remaining_time
        if (is->show_mode != SHOW_MODE_NONE && (!is->paused || is->force_refresh))
            video_refresh(is, &remaining_time); // 显示步骤2 进入绘制
        SDL_PumpEvents();
    }
}
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
        double x;
        refresh_loop_wait_event(cur_stream, &event);  // 显示步骤1. 取出事件 并且做延迟等处理
        switch (event.type) {
        case SDL_KEYDOWN:   
            // 点击或操作等事件 设置标识位或值
...
// 1. 有AVDevice会对其进行初始化
    ...
    is = stream_open(input_filename, file_iformat); // 4. 打开文件
    if (!is) {
        av_log(NULL, AV_LOG_FATAL, "Failed to initialize VideoState!\n");
        do_exit(NULL);
    }

    event_loop(is); // 最后 开始循环
